 ...
 * - "참조은+참조은nb" → 그대로 유지
 */
function normalizeBranchName(raw: string): { branchName: string; partnerName: string } {
  let name = raw.trim();
  let partner = '';
...
function parseOperationType(raw: string): string {
...
function parseConnection(raw: string): { name: string; type: 'student' | 'agency' | null } {
  const v = cleanStr(raw);
 ...
 * - v2: 헤더에 "구분" 있음 (N열 근처)
 */
function detectFormat(headerRow: string[]): 'v1' | 'v2' {
